    ...
    $("#productList").append(`
      <div class="col-md-4">
        <div class="product-card">
          ...
          <h6 style="margin-bottom: 4px">${p.name}</h6>
          <p style="font-size: 13px; color: #555; font-weight: 300; font-size: 13px; margin-bottom: 8px;"><strong>${sku}</strong></p>
          <p style="font-size: 14px; color: #555; " color:${stockColor}">Php ${p.price.toFixed(2)} | Stock: ${stockText}</p>
          <button class="add-btn" ${p.stock===0?'disabled':''} onclick="addToCart('${p.name}')"><i style="color: #fff; " class="bi bi-bag-plus-fill brand-icon"></i> Add</button>
        </div>
      </div>
    `);
  });
